# Code of Conduct

This project adopts the [Contributor Covenant](https://www.contributor-covenant.org),
version 2.1, as its code of conduct.

In short: be respectful and constructive; assume good faith; no harassment,
discrimination, or personal attacks. We want a welcoming space for people of every
background and experience level.

The full text is at
<https://www.contributor-covenant.org/version/2/1/code_of_conduct/>.

## Enforcement

Report unacceptable behavior privately to the maintainer at `aatman@randomness.life`.
Reports are handled confidentially; the maintainer will review and respond as
promptly as is reasonable, and may take any action deemed appropriate, up to and
including a ban from project spaces.

> Status: **pre-release, local-only** — this policy is in place for when the project
> is published and has a community.
