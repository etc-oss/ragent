...
description: ragent splits into (a) the sandbox SUBSTRATE (jail + egress allowlist + caps + Nix tools + optional local model + an ACP/A2A endpoint + a profile SPI) and (b) PROFILES — typed agent kinds (code, web, data, reservations, ops, research) where each profile is a thin wrapper with a standard interface over a PLUGGABLE tool adapter (Firecrawl, an MCP data server, a booking API, the review transport). `code` is the reference profile. The sandbox posture + egress allowlist are per-profile. The profiles are xynergy's syKeepers.
status: accepted
date: 2026-08-16
tags: [architecture, sandbox, profiles, adapters, acp, a2a, xynergy, refactor]
...
- **Profiles** = typed agent kinds. Each is a **thin wrapper with a standard interface**, and
  ...
  ERP/provider adapter for another, with no change to the substrate or to xynergy.
...
## Ties to xynergy + interop

- **The ragent profiles *are* xynergy's syKeepers** (the `web` profile = a web keeper, `data` =
  a data keeper, …). ragent supplies the *sandboxed profile primitives* + an **ACP/A2A** surface;
  **xynergy is the conductor** — board, xDae intake, pipelines, governance, source-of-record, and
  the *outbound* adapters (ERP/CRM/…). ragent = the instruments; xynergy = the score.
- **ACP** (coding-CLI) + **A2A** (framework-to-framework) is the surface every profile exposes, so
  xynergy — or any other orchestrator — drives them uniformly. It's the composability linchpin.

## Consequences / discipline
- Extract the **profile SPI + the ACP/A2A endpoint** now; keep `code` as the reference. Add
  `web`/`data`/… **just-in-time** as xynergy pulls them — each a **thin adapter-wrapper around an
...
- xynergy `docs/CHARTER.md` + its ADR-0003 (the profiles are its syKeepers).
