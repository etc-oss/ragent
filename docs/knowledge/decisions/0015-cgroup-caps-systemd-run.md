...
**user cgroup delegation** (`Delegate=cpu cpuset io memory pids` drop-in in
`lima/ragent.yaml`) so `--user` scopes can actually enforce the limits.
...
- `tools/ragent-run.sh`, `lima/ragent.yaml`
